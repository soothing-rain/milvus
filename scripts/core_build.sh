...
while getopts "b:p:d:t:s:f:ulrcghzme" arg; do
  case $arg in
  f)
...
usage:
./core_build.sh -p \${INSTALL_PREFIX} -t \${BUILD_TYPE} -s \${CUDA_ARCH} -f\${CUSTOM_THIRDPARTY_PATH} -b\${EMBEDDED_MILVUS} [-u] [-l] [-r] [-c] [-z] [-g] [-m] [-e] [-h]
                "
    exit 0
    ;;
  ?)
