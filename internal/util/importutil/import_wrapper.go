...
func (p *ImportWrapper) Import(filePaths []string, rowBased bool, onlyValidate bool) error {
	...
	if p.isBinlogImport(filePaths) {
		return p.doBinlogImport(filePaths)
	}
...
func (p *ImportWrapper) doBinlogImport(filePaths []string) error {
	...
	parser, err := NewBinlogParser(p.collectionSchema, p.shardNum, p.segmentSize, p.chunkManager, flushFunc)
	if err != nil {
		return err
	}
