...
func (p *BinlogAdapter) Read(segmentHolder *SegmentFilesHolder) error {
	...
	for i := 0; i < batchCount; i++ {
		...
		primaryLog := batchFiles[p.primaryKey] // no need to check existence, already verified
		...
		timestampLog := batchFiles[common.TimeStampField] // no need to check existence, already verified
		timestampList, err := p.readInt64Field(timestampLog)
		if err != nil {
			return err
		}
...
func (p *BinlogAdapter) decodeDeleteLogs(segmentHolder *SegmentFilesHolder) ([]*storage.DeleteLog, error) {
	...
	log.Error("Binlog adapter: total deletion count", zap.Int("count", len(stringArray)), zap.String("firstDeletion", stringArray[0]))
	...
	for i := 0; i < len(stringArray); i++ {
		...
		if err := json.Unmarshal([]byte(stringArray[i]), deleteLog); err != nil {
			...
			if len(splits) != 2 {
				log.Error("Binlog adapter: the format of deletion string is incorrect", zap.String("deltaString", stringArray[i]))
				return nil, fmt.Errorf("the format of deletion string is incorrect, %v can not be split", stringArray[i])
			}
	...
	log.Error("Binlog adapter: deletion count after filtering", zap.Int("count", len(deleteLogs)))
...
func (p *BinlogAdapter) getShardingListByPrimaryInt64(primaryKeys []int64,
	...
	for i, key := range primaryKeys {
		// if this item's timestamp is greater than the tsEndPoint, set shardID = -1 to skip
		ts := timestampList[i]
		...
		// if the key exists in intDeletedList, that means this item has been deleted
		if deleted {
