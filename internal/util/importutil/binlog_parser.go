...
func NewBinlogParser(collectionSchema *schemapb.CollectionSchema,
	...
	flushFunc ImportFlushFunc) (*BinlogParser, error) {
...
func (p *BinlogParser) constructSegmentHolders(insertlogRoot string, deltalogRoot string) ([]*SegmentFilesHolder, error) {
	...
	insertlogs, _, err := p.chunkManager.ListWithPrefix(insertlogRoot, true)
	if err != nil {
	...
	for _, insertlog := range insertlogs {
		...
		// ignore the timestamp field, since we will create new one in the importFlushReqFunc() of data_node.go
	...
	if len(deltalogRoot) > 0 {
		deltalogs, _, err := p.chunkManager.ListWithPrefix(deltalogRoot, true)
		if err != nil {
...
func (p *BinlogParser) parseSegmentFiles(segmentHolder *SegmentFilesHolder) error {
	// TODO: handle the timestamp end point passed from client side
	adapter, err := NewBinlogAdapter(p.collectionSchema, p.shardNum, p.segmentSize,
		MaxTotalSizeInMemory, p.chunkManager, p.callFlushFunc, 0)
	if err != nil {
