...
func (t *compactionTrigger) ShouldDoSingleCompaction(segment *SegmentInfo, compactTime *compactTime) bool {
	...
	if float64(totalExpiredRows)/float64(segment.GetNumOfRows()) >= float64(Params.DataCoordCfg.SingleCompactionRatioThreshold) || totalExpiredSize > Params.DataCoordCfg.SingleCompactionExpiredLogMaxSize {
	...
	if float64(totalDeletedRows)/float64(segment.GetNumOfRows()) >= float64(Params.DataCoordCfg.SingleCompactionRatioThreshold) || totalDeleteLogSize > Params.DataCoordCfg.SingleCompactionDeltaLogMaxSize {
